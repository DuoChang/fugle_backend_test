...
export class RateLimitGuard implements CanActivate {
  ...
  async canActivate (
    ...
    const ipRequestRecord: string[] | null = await this.redisUtil.getRequestRecords(ip)
    const idRequestRecord: string[] | null = await this.redisUtil.getRequestRecords(id)
  ...
  getCountIp (ipRequestRecord: string[], expire: Date): number {
    return (ipRequestRecord === null) ? 0 : this.countRequestsInOneMin(ipRequestRecord, expire)
  }

  getCountId (idRequestRecord: string[], expire: Date): number {
  ...
  countRequestsInOneMin (requestRecords: string[], expire: Date): number {
    let count = 0
